...
public class EnviroCowEntity extends PathfinderMob {
	...
	public static void addLivingEntityToBiomes(BiomeLoadingEvent event) {
		if (SPAWN_BIOMES.contains(event.getName()))
			event.getSpawns().getSpawner(MobCategory.CREATURE)
					.add(new MobSpawnSettings.SpawnerData(EnvirocraftModEntities.ENVIRO_COW.get(), 10, 1, 8));
	}

	public EnviroCowEntity(PlayMessages.SpawnEntity packet, Level world) {
		this(EnvirocraftModEntities.ENVIRO_COW.get(), world);
	}

	public EnviroCowEntity(EntityType<EnviroCowEntity> type, Level world) {
		super(type, world);
		xpReward = 0;
		setNoAi(false);
	}
	...
	public static void init() {
		SpawnPlacements.register(EnvirocraftModEntities.ENVIRO_COW.get(), SpawnPlacements.Type.ON_GROUND, Heightmap.Types.MOTION_BLOCKING_NO_LEAVES,
