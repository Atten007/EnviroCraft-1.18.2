...
public class EnviroSheepEntity extends PathfinderMob {
	...
	public static void addLivingEntityToBiomes(BiomeLoadingEvent event) {
		if (SPAWN_BIOMES.contains(event.getName()))
			event.getSpawns().getSpawner(MobCategory.CREATURE)
					.add(new MobSpawnSettings.SpawnerData(EnvirocraftModEntities.ENVIRO_SHEEP.get(), 10, 1, 8));
	}

	public EnviroSheepEntity(PlayMessages.SpawnEntity packet, Level world) {
		this(EnvirocraftModEntities.ENVIRO_SHEEP.get(), world);
	}

	public EnviroSheepEntity(EntityType<EnviroSheepEntity> type, Level world) {
		super(type, world);
		xpReward = 0;
		setNoAi(false);
	}
	...
	public static void init() {
		SpawnPlacements.register(EnvirocraftModEntities.ENVIRO_SHEEP.get(), SpawnPlacements.Type.ON_GROUND, Heightmap.Types.MOTION_BLOCKING_NO_LEAVES,
