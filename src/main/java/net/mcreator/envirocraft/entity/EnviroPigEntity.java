...
public class EnviroPigEntity extends PathfinderMob {
	...
	public static void addLivingEntityToBiomes(BiomeLoadingEvent event) {
		if (SPAWN_BIOMES.contains(event.getName()))
			event.getSpawns().getSpawner(MobCategory.CREATURE)
					.add(new MobSpawnSettings.SpawnerData(EnvirocraftModEntities.ENVIRO_PIG.get(), 10, 4, 4));
	}

	public EnviroPigEntity(PlayMessages.SpawnEntity packet, Level world) {
		this(EnvirocraftModEntities.ENVIRO_PIG.get(), world);
	}

	public EnviroPigEntity(EntityType<EnviroPigEntity> type, Level world) {
		super(type, world);
		xpReward = 0;
		setNoAi(false);
	}
	...
	public static void init() {
		SpawnPlacements.register(EnvirocraftModEntities.ENVIRO_PIG.get(), SpawnPlacements.Type.ON_GROUND, Heightmap.Types.MOTION_BLOCKING_NO_LEAVES,
