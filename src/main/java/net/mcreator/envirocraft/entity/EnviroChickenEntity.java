...
public class EnviroChickenEntity extends PathfinderMob {
	...
	public static void addLivingEntityToBiomes(BiomeLoadingEvent event) {
		if (SPAWN_BIOMES.contains(event.getName()))
			event.getSpawns().getSpawner(MobCategory.MONSTER)
					.add(new MobSpawnSettings.SpawnerData(EnvirocraftModEntities.ENVIRO_CHICKEN.get(), 10, 1, 8));
	}

	public EnviroChickenEntity(PlayMessages.SpawnEntity packet, Level world) {
		this(EnvirocraftModEntities.ENVIRO_CHICKEN.get(), world);
	}

	public EnviroChickenEntity(EntityType<EnviroChickenEntity> type, Level world) {
		super(type, world);
		xpReward = 0;
		setNoAi(false);
	}
	...
	public static void init() {
		SpawnPlacements.register(EnvirocraftModEntities.ENVIRO_CHICKEN.get(), SpawnPlacements.Type.ON_GROUND,
